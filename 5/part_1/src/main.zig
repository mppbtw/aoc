...
const Almanac = struct {
    ...
    pub fn init(a: std.mem.Allocator) Almanac {
        return Almanac{
            .seeds = std.ArrayList(usize).init(a),
...
pub fn main() !void {
    ...
    var i: usize = 0;
    while (i < almanac.seeds.items.len) : (i += 1) {
        // Go through every map in the chain
        var seed = almanac.seeds.items[i];
...
fn parseAlmanac(lines: std.ArrayList([]const u8)) Almanac {
    ...
    var seeds = splitStr(splitStr(lines.items[0], ":").items[1], " ");

    for (seeds.items) |s| {
        a.seeds.append(std.fmt.parseInt(usize, s, 10) catch unreachable) catch unreachable;
    }
